...
def test_mentors_names_contains_names():
    assert "Евгений" in mentors_names[0]
    assert "Филипп" in mentors_names[1]
    assert "Адилет" in mentors_names[2]
    assert "Владимир" in mentors_names[2]
    assert "Татьяна" in mentors_names[3]

def test_pairs_contains_courses():
    assert {"Python-разработчик с нуля", "Java-разработчик с нуля"} in pairs
    assert {"Fullstack-разработчик на Python", "Frontend-разработчик с нуля"} in pairs